...
## 10.10 Avoid pesudo-three-demensional plots
...
```{r}
avg2 <- us_contagious_diseases %>%
  filter(disease=="Smallpox" & weeks_reporting>10) %>% group_by(year) %>%
  summarize(us_rate = sum(count, na.rm = TRUE) / 
              sum(population, na.rm = TRUE) * 10000)
```
```{r}
...
dat2 %>% 
  ...
  geom_line(aes(year, rate, group = state),  color = "grey50", 
            show.legend = FALSE, alpha = 0.2, size = 1) +
  geom_line(mapping = aes(year, us_rate),  data = avg2, size = 1) +
  scale_y_continuous(trans = "sqrt", breaks = c(5, 25, 125, 300)) + 
...
dat3 %>% 
  filter(!is.na(rate)) %>%
  ggplot() +
  geom_line(aes(year, rate, group = disease, col = disease), show.legend = TRUE) +
  ggtitle("Cases in California per 10,000 by disease") + 
  xlab("") + ylab("")
```
...
dat4 <- us_contagious_diseases %>%
  ...
  summarize(us_rate = sum(count)/sum(population))
```
```{r}
dat4 %>% 
  filter(!is.na(us_rate))%>%
  ggplot() +
  geom_line(aes(year, us_rate, group = disease, col = disease), show.legend = TRUE) +
  ggtitle("Cases in US by disease") + 
  xlab("") + ylab("")
```
