...
ref<- NHANES %>%
  ...
  summarize(avg=mean(BPSysAve), sd=sd(BPSysAve))
ref
```
...
ref_avg<- NHANES %>%
  filter(Gender=="female", AgeDecade==" 20-29") %>%
  filter(!is.na(BPSysAve)) %>%
  summarize(avg=mean(BPSysAve), sd=sd(BPSysAve)) %>% pull(avg)
ref_avg
```
...
NHANES %>%
  ...
  group_by(AgeDecade) %>%
  summarise(avg=mean(BPSysAve, na.rm = TRUE), sd=sd(BPSysAve, na.rm = TRUE))
```
...
## 5.8 Organizing data wtih spreadsheets
