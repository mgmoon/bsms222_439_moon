...
How can we compute $S_n$ for various values of n, say $n=1,\dots,25$? Do we write 25 lines of code calling compute_s_n? No, that is what for-loops are for in programming. In this case, we are performing exactly the same task over and over, and the only thing that is changing is the value of n. For-loops let us define the range that our variable takes(in our example n=1,...,25), then change the value and evaluate expression as you loop.
...
Now we can create a plot to search for at pattern:
```{r}
n<-1:m
plot(n,s_n)
```
