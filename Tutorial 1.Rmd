...
summary(d)
```
## 2.2 Difference between data fram and matrix
...
# matrix는 variables들을 같은 class에만 저장할 수 있다는 점에서 다릅니다.
```
## 2.3. Subset and Sort
...
```{r}
nrow(distinct(d, p.Protein))
```
How many unique variants you can find? and which variants are occurred in multiple times?
```{r}
# Variants that occurred multiple times
recurring_var <- c %>% filter(Freq > 1) %>% select(Var1) %>% pull()
recurring_var
```
...
```{r}
# I would use 'Inheritence' to know the relationship between recurrent mutation and inheritence.

d %>% group_by(Inheritence) %>% filter(p.Protein %in% recurring_var) %>% count()
```
```{r}
# Most recurrent mutations occurred in DeNovo.
```

## 3.3 What is the proportion of diagnosis for SCN2A patient?

SCN2A mutation can have multiple different consequences for disease phenotypes. It can cause ASD but also other neurodevelopmental conditions. In total cases, how many phenotypes occur in SCN2A patients.

```{r}
phenotypes <- d %>% group_by(Classification) %>% count()
phenotypes
```

Then, calculate the proportion of the phenotypes among total cases. 
```{r}
d %>% group_by(Classification)%>% count() %>% mutate(proportion_phen = n/sum(phenotypes$n))
```
Then, you might be intrigued to whether females and males have different occurrence in each disorder. Let's check it. 

```{r}
n_female <-sum(d$PatientSex=="F")
n_male <-sum(d$PatientSex=="M")
d %>%
  group_by(Classification, PatientSex) %>%
  filter(PatientSex %in% c("F","M")) %>%
  count() %>%
  mutate(sex_proportion=ifelse(PatientSex=="F", n/n_female, n/n_male))
# There's no significant difference between female and male.
```
...
```{r}
ref <- d %>% group_by(Classification)%>% count(Effect)
ref %>% group_by(Classification) %>% count()
```
...
```{r}
# ASD환자들과 recurrent mutation의 locus
recurring_locus <- d %>%
  group_by(p.Protein)%>%
...
```{r}
d %>% filter(ASD %in% c("Y", "Maybe")) %>%
  group_by(Inheritence, PatientSex) %>%
  count()
```
```{html}
<img src="https://github.com/mgmoon/bsms222_439_moon/blob/main/graph.PNG">
```
