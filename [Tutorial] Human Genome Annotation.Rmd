...
![Figure 1. Comparison of GENCODE and RefSeq gene annotation and the impact of reference geneset on variant effect prediction (Frankish et al. 2015). A)](https://media.springernature.com/full/springer-static/image/art%3A10.1186%2F1471-2164-16-S8-S2/MediaObjects/12864_2015_Article_7216_Fig1_HTML.jpg?as=webp){width=100%}
...
![Figure 2. Comparison of CCDS and Gencode (Source).](https://pbs.twimg.com/media/BiIoBo9IQAA2LQL?format=jpg&name=small){width=100%}
...
ex_d %>% group_by(gene_id) %>% count() %>% head()
```

```{r}
# the mean number of transcripts per gene
ex_d %>% group_by(gene_id) %>% count() %>% pull() %>% mean()
```
```{r}
# the quantile for these numbers
ex_d %>% group_by(gene_id) %>% count() %>% pull() %>% quantile(c(0.25, 0.5, 0.75))
```
```{r}
# gene which has the greatest number of transcript
max_ts <- ex_d %>% group_by(gene_id) %>% count() %>% pull(n) %>% which.max()
ex_d$gene_id[max_ts]
```
...
```{r}
ex_d$gene_type <-as.character(do.call(rbind.data.frame, strsplit(ex_d$info, 'gene_type\\s+"'))[[2]])
ex_d$gene_type <- as.character(do.call(rbind.data.frame,
 strsplit(ex_d$gene_type, '\\"'))[[1]])
ex_d %>%
    group_by(gene_type) %>%
    count()
```
...
ex_d %>%
  ...
  mutate(chr_type = case_when(chrom %in% c("chrX", "chrY") ~ "sex_chromosomes", chrom == "chrM" ~ "chrM",  TRUE ~ "auto_chromosomes")) %>%
    group_by(chr_type) %>% filter(chr_type %in% c("auto_chromosomes", "sex_chr")) %>% summarize(quantile(length))
```
...
```{r}
d2$gene_type <-as.character(do.call(rbind.data.frame, strsplit(d2$info, 'gene_type\\s+"'))[[2]])
d2$gene_type <- as.character(do.call(rbind.data.frame,
 strsplit(d2$gene_type, '\\"'))[[1]])
d2 %>% filter(feature_type == "transcript") %>% group_by(transcript_support_level, gene_type) %>% count()
```
...
```
```{r}
d <- d %>% select(c(gene_id, gene_name, hgnc_id, gene_type, chrom, start, end, strand))
ex_d2<-ifelse(is.na(d$hgnc_id), 0, 1)
ex_d2<-ifelse(is.na(d$tag) | d$tag != "CCDS", 0, 1)
```

2. Please count the number of hgnc by gene biotypes.
```{r}
d %>% group_by(gene_type, hgnc_id) %>% count()
```
...
```{r}
d %>% group_by(d$level, hgnc_id) %>% count()
```
...
```
```{r}
d %>% filter(type=="transcript") %>% count(gene_id) %>% top_n(1, n)
```
...
```{r}
d %>% filter(type=="gene") %>% filter(seqnames!="chrM") %>% mutate(chr_type=ifelse(seqnames%in%c("chrX", "chrY"), "sex", "auto")) %>% group_by(chr_type) %>% count()
```
